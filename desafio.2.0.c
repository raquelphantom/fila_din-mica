...
/*void bubble_sort(fila * f) 
{ 
  int TAMANHO_VETOR = tamanho(f);
  int i, j; int aux;
  for ( j = 0; j<TAMANHO_VETOR-1; j++) 
  {
    for ( i = 0; i<TAMANHO_VETOR-1-j; i++) 
    {
      if ( r[i].PAG < r[i+1].PAG)//esse cara nessa posição é maior que a do lado?
      {
         aux 	   = r[i].PAG;//guarda valor pra não sobrescrever dados
	r[i] .PAG	   = r[i+1].PAG;//troca de lugar
	r[i+1].PAG  = aux;//troca de lugar pt2
      }
    }
   }
}*/

void vetorTemp (fila* f, int tam) 
{  
   int		  *chaves = malloc(sizeof(int) * tam);        
   char       *NOMEDOCs=malloc(sizeof(char) * tam);
   char       *USERs=malloc(sizeof(char) * tam);
   int        *PAGs=malloc(sizeof(int) * tam);
   char       *DATAs=malloc(sizeof(char) * tam);;
   int g = 0;
   PONT end = f->inicio;  
   system ("cls");
   printf("fila:\n");  
   while (end != NULL) 
   {
	printf("[%i-%s-%s-%i-%s-%s]", end->reg.chave, end->reg.NOMEDOC, end->reg.USER, end->reg.PAG,end->reg.DATA);  
	end = end->PROX;
   }
}
...
bool inserirNaFila (fila * f)
{
   ...
   f->fim = novo;
   system ("cls"); system ("mode 110, 12");
   printf ("\nChave [[%i-%s-%s-%i-%s-%s] inserida com sucesso.", 
   elem.chave, elem.NOMEDOC, elem.USER,elem.PAG,elem.DATA );
   return true;
}
...
{	REGISTRO r;
	...
	printf ("\nNOME DO DOCUMENTO:			 "); fflush (stdin); gets(r.NOMEDOC);
	printf ("\nNOME: 		"); fflush (stdin); gets(r.USER);
	printf ("\nPÁGINA:			 "); fflush (stdin);  scanf("%i", &r.PAG);
	printf ("\nDAT E HORA: 		"); fflush (stdin); gets(r.DATA);
	return r;
}
...
bool    insereElemFila      (fila * f, REGISTRO reg)
{
   ...
   reg.chave, reg.NOMEDOC, reg.USER,reg.PAG, reg.DATA);
   return true;
}
  ...
  while (opc != 's')
  {
   	...
   	switch (opc) 
	{
		  ...
		  if (excluirDaFila(F, &r)==true)
      		printf ("\nRegistro excluído: [%i-%s-%s-%i-%s]", 
			r.chave, r.NOMEDOC, r.USER, r.PAG,r.DATA);
      	  else
